...
class ClientMainWindow(QMainWindow):
    ...
    def add_contact_window(self):
        ...
        select_dialog = AddContactDialog(self.client, self.client.database)
    ...
    def delete_contact_window(self):
        ...
        remove_dialog = DelContactDialog(self.client.database)
    ...
    def delete_contact(self, item):
        ...
        try:
            ...
            self.client.database.remove_contact(selected)
            self.clients_list_update()
    ...
    def send_message(self):
        ...
        if not message_text:
            return
        try:
            self.client.message_to_user(self.current_chat, message_text)
            pass
        except OSError as err:
            if err.errno:
                self.messages.critical(self, 'Ошибка', 'Потеряно соединение с сервером!')
            ...
            self.client.database.save_message(self.current_chat, 'out', message_text)
    ...
    def message(self, sender):
        if sender == self.current_chat:
            ...
            if self.client.database.check_contact(sender):
